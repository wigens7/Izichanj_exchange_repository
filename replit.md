...
## Recent Changes
- **Feb 13, 2026**: Integrated MonCash payment gateway for deposits. Users can deposit via USDT crypto or MonCash mobile wallet. Deposit page has two method tabs (USDT Crypto / MonCash). MonCash deposits are auto-approved after payment verification. Backend: OAuth 2.0 token flow, CreatePayment, RetrieveTransactionPayment endpoints. Schema updated with depositMethod enum (usdt/moncash), amountHtg, moncashTransactionId columns on deposits table. Admin panel shows deposit method badge. Full i18n support (EN/FR/HT). MonCash sandbox URL: sandbox.moncashbutton.digicelgroup.com.
