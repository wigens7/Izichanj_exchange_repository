...
- Quick-reply BUTTONS on Telegram: each "New Support Message" notification carries an inline keyboard (`buildQuickReplyKeyboard(convId)`) with `callback_data = qr:<convId>:<qrId>` (≤64 bytes). The webhook handles `callback_query` updates (admin-chat gated, regex-parsed) and sends the saved reply in one tap. **Requires** `allowed_updates` to include `callback_query` — re-run `/api/admin/telegram/connect` (from the production domain) after deploy or taps won't be delivered.
