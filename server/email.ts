...
function parseWhatsAppMessage(message: string): { title: string; bodyHtml: string; bodyText: string } {
  ...
  const escaped = escapeHtml(bodyText)
    .replace(/\*([^*\n]+)\*/g, "<strong>$1</strong>")
    .replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" style="color:#D4AF37;text-decoration:none">$1</a>')
    .replace(/\n/g, "<br>");
...
export async function sendMediaNotificationEmail(to: string, fileUrl: string, fileName: string, caption: string | undefined, name?: string | null) {
  ...
  const attachmentBlock = isImage
    ? `<div style="margin:18px 0"><img src="${escapeHtml(fileUrl)}" alt="${escapeHtml(fileName)}" style="max-width:100%;border-radius:10px;border:1px solid #D4AF37"></div>`
    : `<div style="margin:18px 0;padding:14px;background:#111111;border:1px solid #333;border-radius:10px"><div style="font-size:13px;color:#a0a0a0;margin-bottom:6px">📎 Attachment</div><a href="${escapeHtml(fileUrl)}" style="color:#D4AF37;text-decoration:none;font-weight:500">${escapeHtml(fileName)}</a></div>`;
...
<html><body style="margin:0;padding:0;background:#111111;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;color:#ffffff">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#111111;padding:32px 10px">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#1a1a1a;border:1px solid #D4AF37;border-radius:12px;overflow:hidden;box-shadow:0 4px 15px rgba(0,0,0,0.5)">
        <tr><td style="padding:30px 32px 0 32px">
          <div style="text-align:center;margin-bottom:25px">
            <h1 style="color:#D4AF37;margin:0;font-size:30px;font-weight:700;letter-spacing:2px;text-transform:uppercase">Izichanj</h1>
            <div style="height:2px;width:60px;background-color:#D4AF37;margin:10px auto 0 auto"></div>
          </div>
        </td></tr>
        <tr><td style="padding:0 32px 32px 32px">
          <h2 style="color:#ffffff;font-size:22px;font-weight:600;margin-top:0;margin-bottom:6px">${escapeHtml(opts.subject)}</h2>
          ...
          <p style="color:#a0a0a0;font-size:12px;line-height:1.6;text-align:center;margin-top:30px;border-top:1px solid #333;padding-top:20px">
            ...
            You can unsubscribe anytime from your Izichanj profile page.<br>
            &copy; 2026 Izichanj. All rights reserved.
          </p>
        </td></tr>
      </table>
